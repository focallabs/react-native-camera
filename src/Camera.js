...
function convertNativeProps(props) {
  ...
  if (typeof props.totalSeconds === 'number') {
    newProps.totalSeconds = props.totalSeconds;
  }
...
export default class Camera extends Component {
  ...
  getSoundLevel() {
    ...
    return CameraManager.getSoundLevel();

  }

  initSoundLevel() {
    return CameraManager.initSoundLevel();
  }

  releaseSoundLevel() {
    CameraManager.releaseSoundLevel()
  }

  // Tung Duong: function to call to Android native to set exposure compensation
  setExposureCompensation(options) {
    ...
    CameraManager.setExposureCompensation(options)
  }

  getExposureCompensationRange(options) {
    const props = convertNativeProps(this.props);
    options = {
      type: props.type,
      ...options,
    };
    return CameraManager.getExposureCompensationRange(options)
  }
  
  // Function to calll iOS native to set ISO for camera

  getISORange() {
    return CameraManager.getISORange()
  }

  getCurrentISO(){
    return CameraManager.getCurrentISO();
  }

  getCurrentAudioLevel(){
    return CameraManager.getAudioLevel();
  }

  changeISO(value) {
    return CameraManager.changeISO(value);
  }
