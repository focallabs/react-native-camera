...
public class RCTCameraModule extends ReactContextBaseJavaModule
    ...
    private MediaRecorder mCrewMediaRecorder;
    private File mCrewTempAudioFile;
    private boolean mIsMediaRecorderReleasing;
    ...
    MediaRecorder.OnInfoListener onInfoListener = new MediaRecorder.OnInfoListener() {
        @Override
        public void onInfo(MediaRecorder mr, int what, int extra) {
            if (what == MediaRecorder.MEDIA_RECORDER_INFO_MAX_DURATION_REACHED || what == MediaRecorder.MEDIA_RECORDER_INFO_UNKNOWN) {
    ...
    private Throwable prepareMediaRecorder(ReadableMap options, int deviceOrientation) {
        if (mCrewMediaRecorder != null) {
            releaseCrewMediaRecorder();
        }
        ...
        int mediaRecorderHintOrientation = 0;
        switch (deviceOrientation) {
            case 0:
                mediaRecorderHintOrientation = 90;
                break;
            case 1:
                mediaRecorderHintOrientation = 0;
                break;
            case 2:
                mediaRecorderHintOrientation = 270;
                break;
            case 3:
        ...
        if (RCT_CAMERA_TYPE_FRONT == options.getInt("type")) {
            if ((RCT_CAMERA_ORIENTATION_PORTRAIT == RCTCamera.getInstance().getOrientation())) {
                mediaRecorderHintOrientation += 180;
            }
        }
    ...
    private Throwable prepareMediaRecorderCrewstudio() {
        if (mCrewMediaRecorder != null) {
            releaseCrewMediaRecorder();
        }
        try {
            mCrewMediaRecorder = new MediaRecorder();
            mCrewMediaRecorder.setAudioSource(MediaRecorder.AudioSource.MIC);
            mCrewMediaRecorder.setOutputFormat(MediaRecorder.OutputFormat.THREE_GPP);
            mCrewMediaRecorder.setAudioEncoder(MediaRecorder.OutputFormat.AMR_NB);
            mCrewTempAudioFile = getTempMediaFile(MEDIA_TYPE_AUDIO);
            if (mCrewTempAudioFile == null) {
                return new RuntimeException("Error while preparing output file in prepareMediaRecorder.");
            }
            mCrewMediaRecorder.setOutputFile(mCrewTempAudioFile.getPath());

            try {
                mCrewMediaRecorder.prepare();
            } catch (Exception ex) {
                Log.e(TAG, "Media recorder prepare error.", ex);
                releaseCrewMediaRecorder();
                return ex;
    ...
    private void recordLevel(final Promise promise) {

        Throwable prepareError = prepareMediaRecorderCrewstudio();
        if (prepareError != null) {
            promise.reject(prepareError);
            return;
        }
    ...
    private void releaseMediaRecorder() {
        ...
        if (this.mIsMediaRecorderReleasing) {
            return;
        }
        this.mIsMediaRecorderReleasing = true;
        try {
        ...
        this.mIsMediaRecorderReleasing = false;
    }

    private void releaseCrewMediaRecorder() {
        ...
        try {
            ...
            if (mCrewMediaRecorder != null) {
                // Stop recording video.
                try {
                    mCrewMediaRecorder.stop(); // stop the recording
                } catch (RuntimeException ex) {
                    Log.e(TAG, "Media recorder stop error.", ex);
                }

                // Optionally, remove the configuration settings from the recorder.
                mCrewMediaRecorder.reset();

                // Release the MediaRecorder.
                mCrewMediaRecorder.release();

                // Reset variable.
                mCrewMediaRecorder = null;
            }


            File f = new File(mCrewTempAudioFile.getPath());
            if (!f.exists()) {
                return;
            }
    ...
    @ReactMethod
    public void initSoundLevel(final Promise promise) {
//        Log.d("TungDuong", "soundLevel");
        recordLevel(promise);
    }

    @ReactMethod
    public void releaseSoundLevel() {
//        Log.d("TungDuong", "releaseSoundLevel");
        releaseCrewMediaRecorder();
    }

    @ReactMethod
    public void setExposureCompensation(final ReadableMap options) {
        RCTCamera camera = RCTCamera.getInstance();
        camera.setExposureCompensation(options.getInt("type"), options.getInt("expo"));
    }

    // Tung Duong function to get min max exposure compensation
    @ReactMethod
    public void getExposureCompensationRange(final ReadableMap options, final Promise promise) {

        RCTCamera camera = RCTCamera.getInstance();
        if (camera == null) {
            promise.reject(new Throwable("Cannot get camera"));
            return;
        }
        int[] minMax = camera.getExposureCompensationRange(options.getInt("type"));
        if (minMax != null) {
            WritableMap map = Arguments.createMap();
            map.putDouble("min", minMax[0]);
            map.putDouble("max", minMax[1]);
            promise.resolve(map);
        } else {
            promise.reject(new Throwable("Cannot get exposure compensation min max"));
        }
    }

    @ReactMethod
    public void getSoundLevel(final Promise promise) {

        if (mCrewMediaRecorder != null) {
            processSoundLevel(mCrewMediaRecorder, promise);
            return;
        } else if (mMediaRecorder != null) {
            processSoundLevel(mMediaRecorder, promise);
            return;
        }

        promise.reject(new Throwable("MediaRecorder doesn't finish initial"));
    }

    private void processSoundLevel(MediaRecorder mediaRecorder,final Promise promise) {
        try {
            int level = mediaRecorder.getMaxAmplitude();
            double decibel = maxAmplitudeToDecibel(level);
    ...
    private double maxAmplitudeToDecibel(int maxAmplitude) {
//        if (maxAmplitude <= 0) {
//            return 0;
//        }
        double decibel = (double) ((maxAmplitude * 100) / 32767) ;
    ...
    private void captureWithOrientation(final ReadableMap options, final Promise promise, int deviceOrientation) {
        ...
        int orientation = options.hasKey("orientation") ? options.getInt("orientation") : RCTCamera.getInstance().getOrientation();
        if (orientation == RCT_CAMERA_ORIENTATION_AUTO) {
            // Tung Duong: Add check device orientation if not landscape left/right make orientation become landscape-left
            if (deviceOrientation != RCT_CAMERA_ORIENTATION_LANDSCAPE_LEFT && deviceOrientation != RCT_CAMERA_ORIENTATION_LANDSCAPE_RIGHT) {
                deviceOrientation = RCT_CAMERA_ORIENTATION_LANDSCAPE_LEFT;
            }
        }
...
//            environmentDirectoryType = _reactContext.getFilesDir().getAbsolutePath();
    ...
    private File getTempMediaFile(int type) {
        try {
            ...
            if (type == MEDIA_TYPE_IMAGE) {
                ...
                outputFile = File.createTempFile("AUD_" + timeStamp, ".mp3", outputDir);
            } else {
                Log.e(TAG, "Unsupported media type:" + type);
                return null;
            }
