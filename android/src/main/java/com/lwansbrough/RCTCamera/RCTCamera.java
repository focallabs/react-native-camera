...
public class RCTCamera {
    ...
    public void setExposureCompensation(int cameraType, int value) {
        try {
            Camera camera = _cameras.get(cameraType);
            if (camera == null) {
                return;
    ...
    public int[] getExposureCompensationRange(int cameraType) {
        try {
            int[] result = null;
            Camera camera = _cameras.get(cameraType);
            if (camera == null) {
    ...
    public void adjustPreviewLayout(int type) {
        ...
        Camera.Size optimalPreviewSize = getBestSize(parameters.getSupportedPreviewSizes(), RESOLUTION_1080P.width, RESOLUTION_1080P.height);
