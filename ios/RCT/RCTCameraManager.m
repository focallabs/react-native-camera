...
-(void)getAudioLevel {
    NSTimer *t = [NSTimer scheduledTimerWithTimeInterval: 1.0
                                                  target: self
                                                selector:@selector(calculateAudioLevel)
...
-(float)calculateAudioLevel {
    ...
    float audioLevel = (pow(10.f, 0.05f * decibels) * 20.0f);
    float audioLevelValue = isnan(audioLevel) ? 0 : audioLevel;
//    NSLog(@"Audio level : %f dB ",audioLevelValue);
...
- (void)initializeCaptureSessionInput:(NSString *)type {
    dispatch_async(self.sessionQueue, ^{
        ...
        if (type == AVMediaTypeAudio) {
            ...
            self.captureDevice = [self deviceWithMediaType:AVMediaTypeVideo preferringPosition:self.presetCamera];
        }
        ...
        AVCaptureDeviceInput *captureDeviceInput = [AVCaptureDeviceInput deviceInputWithDevice:self.captureDevice error:&error];
...
RCT_EXPORT_METHOD(getAudioLevel:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject) {
    float audioLevel = [self calculateAudioLevel];
    resolve(@(audioLevel));
}

RCT_EXPORT_METHOD(getISORange:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject) {
    AVCaptureDevice *currentDevice = [self.videoCaptureDeviceInput device];
    float maxISO = currentDevice.activeFormat.maxISO;
    float minISO = currentDevice.activeFormat.minISO;
    float currentISO = (maxISO - currentDevice.ISO);
    if (currentISO < minISO) { currentISO = minISO; }
    if (currentISO > maxISO) { currentISO = maxISO; }
//    if (currentISO > minISO) {
//        currentISO = [self convertISOCurve:currentISO];
//    }
    resolve(@{@"max":@(maxISO),@"min":@(minISO),@"current":@(currentISO)});
}

-(double)convertISOCurve:(double)x {
    return  (56.51327 + 0.9938079*x + 0.00225547*x*x - 0.00001216135*x*x*x + 0.00000001207129*x*x*x*x);
}

RCT_EXPORT_METHOD(changeISO:(int)value) {
#if !(TARGET_IPHONE_SIMULATOR)
    NSError *error = nil;
    self.captureDevice = [self.videoCaptureDeviceInput device];
    
    if (value <= 0 || value < round(self.captureDevice.activeFormat.minISO) || value > round(self.captureDevice.activeFormat.maxISO) ) {return;}
    
    if ( [self.captureDevice lockForConfiguration:&error] ) {
        if ([self.captureDevice isExposureModeSupported:AVCaptureExposureModeCustom]) {
            //Set customize before can make any config
